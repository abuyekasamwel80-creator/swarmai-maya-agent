 ...
 *
 * Polls OKX's public REST API at 500ms intervals — no API key required.
 * OKX endpoint: https://www.okx.com/api/v5/market/books?instId=BTC-USDT&sz=20
 *
 * Symbol mapping: BTCUSDT → BTC-USDT (OKX format)
 */
...
function toOkxInst(sym: string): string {
  ...
  for (const stable of stablecoins) {
    if (s.endsWith(stable)) {
      const base = s.slice(0, s.length - stable.length);
      return `${base}-${stable}`;
    }
  }
...
class OrderBookService extends EventEmitter {
  ...
  getBook(symbol: string): OrderBook | null {
    return this.books.get(symbol.toUpperCase().replace("/", "")) ?? null;
  }
...
// Auto-subscribe to top pairs on startup
