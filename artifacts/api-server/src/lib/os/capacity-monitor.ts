/**
 * SwarmAI Agent OS — Capacity Monitor
 *
 * Real-time throughput tracking and fleet health.
 */
...
export interface FleetHealth {
  ...
  nvidia: ReturnType<typeof getProviderStats>;
  openrouter: ReturnType<typeof getProviderStats>;
...
export function getFleetHealth(): FleetHealth {
  ...
  const openrouterModels = ALL_MODELS.filter(
  ...
  for (const [id, s] of Object.entries(allStats)) {
    currentInflight += s.inflight;
    currentQueued += s.queued;
    if (s.inflight > 0 || s.queued > 0) {
      busy.push({ id, inflight: s.inflight, queued: s.queued });
    }
  }
  ...
  const utilizationPct =
    totalTheoreticalRpm > 0
      ? Math.round((currentInflight / (totalTheoreticalRpm / 60)) * 100)
      : 0;

  return {
    totalModels: nvidiaModels + openrouterModels,
    nvidiaModels,
    openrouterModels,
    totalTheoreticalRpm,
    nvidiaRpm: nvidia.totalRpm,
    openrouterRpm: openrouter.totalRpm,
    currentInflight,
    currentQueued,
    utilizationPct,
    nvidia,
    openrouter,
    topBusyModels: busy.slice(0, 10),
  };
}
